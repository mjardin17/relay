...
async function ensureServerRunning() {
  try {
    const res = await fetch(`${TEST_BASE_URL}/api/gbp-launch/connector-status`, {
      headers: { Authorization: 'Bearer token_owner_tenant1' },
    });
    if (res.ok) {
      console.log(`Server is already running on port ${TEST_PORT}.`);
      return;
    }
  } catch {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    try {
      ...
      if (res.ok) {
        console.log(`Test server is ready on port ${TEST_PORT}.`);
        return;
      }
    } catch {}
  }
...
async function runFullReleaseAudit() {
  ...
  try {
    ...
    // Ensure server is running for API test suites
  ...
  // 7b. Data Reconciliation & Compliance Boundary Suite
  runAndCapture('Reconciliation & Compliance Boundaries Suite', 'npx tsx scripts/test-reconciliation-and-boundaries.ts');
